...
public class ItemCrystalCaster extends ItemTCBase implements IArchitect, ICaster
{
    int area;
    DecimalFormat myFormatter;
    ArrayList<BlockPos> checked;
    
    public ItemCrystalCaster(String name, int area) {
        super(name);
        this.area = 0;
        ...
        addPropertyOverride(new ResourceLocation("focus"), new IItemPropertyGetter() {
            ...
            public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) {
                ItemFocus f = ((ItemCrystalCaster)stack.getItem()).getFocus(stack);
                if (stack.getItem() instanceof ItemCrystalCaster && f != null) {
                    return 1.0f;
                }
                return 0.0f;
            }
    ...
    public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
        if (oldStack.getItem() != null && oldStack.getItem() == this && newStack.getItem() != null && newStack.getItem() == this) {
            ItemFocus oldf = ((ItemCrystalCaster)oldStack.getItem()).getFocus(oldStack);
            ItemFocus newf = ((ItemCrystalCaster)newStack.getItem()).getFocus(newStack);
            int s1 = 0;
            int s2 = 0;
            if (oldf != null && oldf.getSortingHelper(((ItemCrystalCaster)oldStack.getItem()).getFocusStack(oldStack)) != null) {
                s1 = oldf.getSortingHelper(((ItemCrystalCaster)oldStack.getItem()).getFocusStack(oldStack)).hashCode();
            }
            if (newf != null && newf.getSortingHelper(((ItemCrystalCaster)newStack.getItem()).getFocusStack(newStack)) != null) {
                s2 = newf.getSortingHelper(((ItemCrystalCaster)newStack.getItem()).getFocusStack(newStack)).hashCode();
            }
            return s1 != s2;
        }
