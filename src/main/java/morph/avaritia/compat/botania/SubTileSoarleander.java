...
public class SubTileSoarleander extends SubTileGenerating {
    ...
    public void onUpdate() {
        ...
        if (getWorld().isRemote) {
            if (burnTime > 0 && supertile.getWorld().rand.nextInt(10) == 0) {
                Vec3d offset = getWorld().getBlockState(getPos()).getOffset(getWorld(), getPos()).add(new Vec3d(0.4, 0.7, 0.4));
                supertile.getWorld().spawnParticle(EnumParticleTypes.VILLAGER_HAPPY, supertile.getPos().getX() + offset.x + Math.random() * 0.2, supertile.getPos().getY() + offset.y, supertile.getPos().getZ() + offset.z + Math.random() * 0.2, 0.0D, 0.0D, 0.0D);
            }
            return;
        }
        ...
        if (linkedCollector != null) {
            if (burnTime == 0) {
                if (mana < getMaxMana()) {
                    for (EntityChicken chicken : supertile.getWorld().getEntitiesWithinAABB(EntityChicken.class, new AxisAlignedBB(supertile.getPos().add(-RANGE, -RANGE, -RANGE), supertile.getPos().add(RANGE + 1, RANGE + 1, RANGE + 1)))) {
                        if (!chicken.isDead) {
                            this.burnTime = 10000;
                            chicken.setDead();
                            supertile.getWorld().playSound(null, supertile.getPos(), SoundEvents.ENTITY_CHICKEN_DEATH, SoundCategory.BLOCKS, 0.2F, 1F);
                            getWorld().addBlockEvent(getPos(), getWorld().getBlockState(getPos()).getBlock(), START_BURN_EVENT, chicken.getEntityId());
                            sync();
                            return;
                        }
                    }
    ...
    public boolean receiveClientEvent(int event, int param) {
        if (event == START_BURN_EVENT) {
            ...
            if (entity != null) {
                entity.world.spawnParticle(EnumParticleTypes.VILLAGER_ANGRY, entity.posX, entity.posY, entity.posZ, 0.0D, 0.0D, 0.0D);
            }
            return true;
        } else {
